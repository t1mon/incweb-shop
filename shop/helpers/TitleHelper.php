...

{
    ...
    {
        if (\Yii::$app->controller->action->id == 'index' || \Yii::$app->controller->action->id == 'product' || \Yii::$app->controller->action->id == 'post')
            return is_string(end($array)) ? end($array) : '';
        if (\Yii::$app->controller->id == 'shop/catalog') {
        ...
        if (\Yii::$app->controller->id == 'blog/post') {
            if (\Yii::$app->controller->action->id == 'category') {
                $categoryName = is_string(end($array)) ? end($array) : null;
                if (!$categoryName)
                    throw new NotFoundHttpException('Category Name is NULL .');
                $category = \shop\entities\Blog\Category::find()->where(['name' => $categoryName])->one();
                return $category ? $category->title : $category->name;
            }
            ...
            if (\Yii::$app->controller->action->id == 'tag') {
                ...
                if (!$tagName)
                    throw new NotFoundHttpException('Tag Name is NULL .');
                $tag = \shop\entities\Blog\Tag::find()->where(['name' => $tagName])->one();
                return $tag->name;
            }
